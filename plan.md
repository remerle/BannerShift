...
The application is non-intrusive: it does not replace the notification system, does not render its own banners, does not subclass or inject anything into other processes, and does not store or read the content of notifications. It only observes when banners appear and moves the operating system's own banner window to a different point on the screen.
...
Three small pieces of state are persisted, each as a single key in standard per-user preferences storage:

- The selected position (string identifier, one of the nine).
- Whether the menu-bar icon is hidden (boolean).
- Whether debug logging is enabled (boolean).

There is no separate preferences file, no JSON, no schema. All three keys are independent. Default values are: position = Top Middle, icon hidden = false, debug logging = false.
...
- Per-application position rules (e.g., "show Slack banners in the top-left and everything else in the top-right").
...
## 22. Re-Implementation Checklist
...
5. On each event, find any banner element by subrole, capture a baseline frame the first time it is seen, compute a target window origin for the user's chosen 3×3 cell on the correct display, snap it to integer points, and apply it via the accessibility position attribute.
...
9. Persist the chosen position, the icon-visibility preference, and the debug-logging preference.
10. Expose a menu-bar item with: position picker, test-notification action, launch-at-login toggle, hide-icon action, about, quit.
11. Log info/error to the unified logger and a permission-restricted file; log debug only when explicitly enabled.
12. Ship as a Developer-ID-signed, notarized, stapled application targeting the same macOS version range.
