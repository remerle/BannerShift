...
/// debounced passes do not recompile the same patterns (perf-002). It is
/// owned by `AppDelegate` (one instance shared with `BannerMover`) and by
/// the rule editor (its own instance), so the cache is amortized across
/// all matches against the same rule set.
///
/// In production all calls are made on the main thread, but the cache is
/// guarded by a lock so a single instance can also be exercised safely
/// from parallel unit tests (Swift Testing runs `@Test` functions on a
/// concurrent executor) and to keep the type robust against future
/// callers on background queues.
public final class RuleMatcher {
  ...
  /// Drop all cached compiled patterns. Safe to call at any time; the
  /// next match call repopulates lazily. With the keyed-cache approach
  /// (entries are correct as long as the pattern string is unchanged)
  /// this is not strictly required, but exposed for callers that want
  /// to bound memory after large rule-set edits.
  ...
  /// Compile-or-fetch a regex for `pattern`, holding the lock only across
  /// the cache lookup/insert. Regex compilation itself happens outside
